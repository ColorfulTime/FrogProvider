...
MI_INLINE MI_Result MI_CALL XYZ_Frog_Set_Wight(
    XYZ_Frog* self,
    MI_Uint32 x)
{
    ((MI_Uint32Field*)&self->Wight)->value = x;
    ((MI_Uint32Field*)&self->Wight)->exists = 1;
    return MI_RESULT_OK;
}

MI_INLINE MI_Result MI_CALL XYZ_Frog_Clear_Wight(
    XYZ_Frog* self)
{
    memset((void*)&self->Wight, 0, sizeof(self->Wight));
    return MI_RESULT_OK;
}
...
public:
    ...
    //
    // XYZ_Frog_Class.Wight
    //
    
    const Field<Uint32>& Wight() const
    {
        const size_t n = offsetof(Self, Wight);
        return GetField<Uint32>(n);
    }
    
    void Wight(const Field<Uint32>& x)
    {
        const size_t n = offsetof(Self, Wight);
        GetField<Uint32>(n) = x;
    }
    
    const Uint32& Wight_value() const
    {
        const size_t n = offsetof(Self, Wight);
        return GetField<Uint32>(n).value;
    }
    
    void Wight_value(const Uint32& x)
    {
        const size_t n = offsetof(Self, Wight);
        GetField<Uint32>(n).Set(x);
    }
    
    bool Wight_exists() const
    {
        const size_t n = offsetof(Self, Wight);
